...
export const adminLogin = async (username: string, password: string) => {
  try {
    ...
    const { data: adminData, error: adminError } = await supabase
      .from('admins')
      .select('username')
      .eq('username', username as unknown as any)
      .eq('password', password as unknown as any) // Note: In production, you should use hashed passwords
      .single();
      ...
      if (error.message.includes('Invalid login credentials')) {
        const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
          ...
          options: {
            ...
            // Configure emailRedirectTo instead of redirectTo for GitHub Pages
